...
Sometimes you may see properties implemented in the Java bean property stile.
...
Properties in Gradle's core plugins that are still of this style will be migrated to managed properties in the future versions.
...
- link:{javadocPath}/org/gradle/api/model/ObjectFactory.html[ObjectFactory] - Allows model objects to be created. See <<#object_factory>> for more details.
